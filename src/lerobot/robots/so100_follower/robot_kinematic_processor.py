...
class EEReferenceAndDelta(RobotActionProcessorStep):
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        # print("--------------------------------")
        # print(f"Step2: Now obs: {t_curr[:3, 3]}")
...
class EEBoundsAndSafety(RobotActionProcessorStep):
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        # print(f"Step3: Action: {pos}")

        # Check for jumps in position
        if self._last_pos is not None:
            dpos = pos - self._last_pos
            n = float(np.linalg.norm(dpos))
            # print(f"Step3: Last Action: {self._last_pos}")
            # print(f"Step3: Dleta Action(Now Action - Last Action) {n}, self.max_ee_step_m: {self.max_ee_step_m}")
